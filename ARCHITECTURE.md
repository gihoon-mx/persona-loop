...
**페르소나의 단위는 문항이 아니라 사람이다.** 문항별 집계가 아니라, 한 응답자가 문항 전체에 어떻게 답했는지(응답 프로필)를 읽어 그 사람을 세운다. (v0.5에서 전환)

**왜 집계 기반을 버렸나.** v0.4까지는 "35명 중 71%가 X라고 답함" 같은 집계를 근거로 페르소나를 만들었다. 이 방식에는 고칠 수 없는 결함이 있다.

- **통계 요약은 사람이 아니다.** X를 고른 71%와 Y를 고른 64%를 동시에 만족하는 응답자가 실제로는 한 명도 없을 수 있다. 다수 의견을 모아 만든 페르소나는 아무도 아닌 사람이고, 아무도 아닌 사람은 유사 서비스 앞에서 어떻게 행동할지 대변할 수 없다.
- **평균은 모순을 지운다.** 집계 페르소나는 언제나 일관적인데, 일관적인 사람은 없다. 이 매끄러움이 "그럴듯하지만 쓸모없는" 페르소나의 정체다.
- **소수가 사라진다.** 35명 중 1명의 답은 어떤 집계에도 잡히지 않는다. 제품 결정에 가장 쓸모 있는 신호가 반올림으로 없어진다.

집계를 버린 것은 아니다. **강등했다** — 집계는 "이 사람이 다수인지 소수인지"를 알려주는 보조 정보(`populationNote`, `evidence[].populationContext`)이고, 페르소나의 실체는 개인의 응답 프로필이다.

**구조** (`packages/core/schemas/persona.schema.json`)

- **`basis`** — `individual`(응답자 1명) 또는 `cluster`(응답 패턴이 닮은 여러 명). **cluster여도 중심 응답자 한 명(`primary`)을 두고 그 사람을 기준으로 쓴다.** 여러 명을 평균 내면 집계 페르소나로 되돌아가기 때문이다. 나머지는 `supporting`으로 기록해 대표성을 보인다.
- **`answerProfile`** — 중심 응답자가 실제로 낸 답변 전체. 뷰어에서 페르소나 옆에 나란히 놓여 **"이건 창작이 아니다"를 증명**한다. 원문(`answer`·`comment`)과 해석(`readAs`)을 필드로 분리해, 어디까지가 응답자의 말이고 어디부터가 모델의 읽기인지 드러낸다.
- **`behaviorModel`** — 페르소나의 목적은 묘사가 아니라 **대변**이다. 이 사람이 유사 서비스를 처음 만나면 무엇부터 하고(`firstMoves`), 무엇으로 판단하고(`decisionDrivers`), 어디서 즉시 나가는지(`dealBreakers`), 얼마나 버티는지(`effortTolerance`)를 규칙으로 적는다. demo-reviewer(M07)가 이 규칙을 그대로 실행하므로, 근거 없이 지어낸 항목 하나가 곧 가짜 리뷰 한 편이 된다.
- **`contradictions`** — 서로 어긋나는 답을 감추지 않고 적는다. 실제 사람은 일관적이지 않고, 이 긴장이 세션에서 "말로는 A라 했지만 실제로는 B를 눌렀다"로 드러난다.
