...
| M02 | core | `packages/core/` | 인증·**접근 게이트(`requireAdmin`)**·Firestore 데이터 API·스키마·공용 스타일 + **응답자 프로필 로직**(`respondent-profile.js` — DOM·Firebase 비의존, 화면과 검증 도구가 같은 함수를 쓴다) | ✅ v0.5.1 |
...
- **페르소나를 저장하기 전에 `node tools/verify-personas.mjs`를 통과시킨다.** 근거의 questionId·respondentLabel 실존, 인용과 실제 답변의 일치, `answerProfile` ↔ 응답 원본 일치, 집계 재현을 기계적으로 검사한다. 빌더의 자기 보고를 믿지 않는 것이 이 도구의 목적이므로 M05가 붙은 뒤에도 생성 결과에 그대로 돌린다. 산문으로 적은 교차표 주장("N명 중 M명")은 총원과 상한만 검사되므로 **사람이 함께 읽어야 한다.**
