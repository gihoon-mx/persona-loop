...
| 1 | 서베이 임포트 툴 + 결과 대시보드 (now-here 40문항×35명 실데이터는 Firebase 연결 후 임포트) | 🔶 툴 완성 |
| 2 | 페르소나 빌더 (서베이 → 페르소나, 근거 인용 포함) — 뷰어는 완성 | |
| 3 | 데모 리뷰어 (Agent가 실제 브라우저로 데모 사용 + 리뷰 작성) — 뷰어·세션 리플레이는 완성 | |
| 4 | 서베이 스튜디오 (생성·공개 응답 폼 ✅ / 코칭 기능 예정) | 🔶 |
